...
import uk.gov.hmrc.ofstedformsproxy.connectors.SoapPost
...
import scalaz._
import Scalaz._
import javax.xml.soap.{MessageFactory, SOAPMessage, SOAPPart}
import uk.gov.hmrc.ofstedformsproxy.models.SoapMessageException.SoapMessageExceptionMonoid._
...
trait CygnumConnector {
  ...
  def getUrn()(implicit hc: HeaderCarrier, ex: ExecutionContext) : String \/ Option[URN]
  def send(p : String)(implicit hc: HeaderCarrier, ex: ExecutionContext): Future[SubmissionFailure \/ SubmissionSuccess]
  def bc()(implicit hc: HeaderCarrier, ex: ExecutionContext) : Unit
}
...
class CygnumConnectorImpl @Inject()(auditConnector: AuditConnector,
  ...
  def soapMessageToString(message: SOAPMessage): String = {
...
//    file.deleteOnExit()
  ...
  def convert(soapMessage : SOAPMessage) : String \/ String = {
...
//    println("""<?xml version="1.0" encoding="utf-8"?>""" + Utility.trim(xx).toString)
//    println("Sep")
//    println("""<?xml version="1.0" encoding="utf-8"?>""" + xx.toString)
//    \/-("""<?xml version="1.0" encoding="utf-8"?>""" + Utility.trim(xx).toString)

    val writer = new StringWriter
    XML.write(writer, xx, StandardCharsets.UTF_8.toString, xmlDecl = true, null)
    val vvv = writer.toString
    println(vvv)
    \/-(vvv)
  }
  ...
  override def send(payload : String)(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[SubmissionFailure \/ SubmissionSuccess] = {
    System.setProperty("javax.xml.soap.MessageFactory", "com.sun.xml.internal.messaging.saaj.soap.ver1_2.SOAPMessageFactory1_2Impl")
    System.setProperty("javax.xml.bind.JAXBContext", "com.sun.xml.internal.bind.v2.ContextFactory")
    sp.post(cygnumUrl, payload, Seq("Content-Type" -> "application/soap+xml; charset=utf-8")).map[SubmissionFailure \/ SubmissionSuccess] {
      response =>
  ...
  override def bc()(implicit hc: HeaderCarrier, ec: ExecutionContext)  = {
    proxyClient.get("http://www.bbc.com")
  }
